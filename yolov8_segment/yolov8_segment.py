...
logging.basicConfig(
    ...
    datefmt='%H:%M:%S')
logger = logging.getLogger("RDK_YOLO_SEG_ROS")
...
class YOLOv8SegROSNode(Node):
    ...
    def publish_rectangle(self, box_points, header, class_id, score):
        ...
        self.get_logger().info(f"Published rectangle for class {class_id} with score {score:.2f}")
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # 渲染结果
            self.get_logger().info(f"Detected {len(ids)} objects")
            ...
            for i, (class_id, score, bbox, corp, mc) in enumerate(zip(ids, scores, bboxes, corpes, mces)):
                # 绘制检测框
                x1, y1, x2, y2 = bbox
                self.get_logger().info(f"({x1}, {y1}, {x2}, {y2}) -> object: {score:.2f}")
                ...
                if x2_corp > x1_corp and y2_corp > y1_corp:
                    ...
                    if len(mask_points) > 0:
                        ...
                        # 进一步过滤：只保留x坐标在原始宽度1/3到2/3之间的点
                        original_width_160 = 160  # 注意：这是160x160坐标系
                        x_min_threshold = original_width_160 // 6
                        x_max_threshold = original_width_160 * 5 // 6
            ...
            # 创建组合图像（原图 + 0.3倍掩码）
            add_result = np.clip(cv_image + 0.3 * zeros_resized, 0, 255).astype(np.uint8)
            ...
            self.get_logger().info(f"Published combined segmentation result for {len(ids)} objects")
